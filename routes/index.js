...
exports.api_songs = function(request, response) {
  ...
  //SELECT rowid, title FROM songs WHERE title LIKE '%Girl%'

  let sql = "SELECT rowid, title, composer FROM songs WHERE title LIKE ? AND composer LIKE ? LIMIT 30"
  ...
  if (urlObj.query['composer']) {
    ...
    console.log("finding composer: " + composer)
  }
...
exports.api_song_details = function(request, response) {
  ...
  let sql = "SELECT rowid, studentnum, title, composer, bookcode, page, length FROM songs WHERE rowid = ?"
